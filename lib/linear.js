...
async function buildModel(cfg) {
  ...
  // Rename-proof matching: a milestone's leading code (the part before "·", e.g. "M1",
  // "M-PROOF", "P-Launch") is stable even when its descriptive suffix is reworded in
  // Linear. Match exact name first, then fall back to this code. Names without "·"
  // (e.g. "Mac v1 Alpha") only match exactly.
  const codeOf = (name) => (name && name.includes('·') ? name.split('·')[0].trim() : '');
  const byCode = {};
  for (const g of groups) { const c = codeOf(g.name); if (c && !(c in byCode)) byCode[c] = g; }
  const matchMilestone = (ms) => ms && (byName[ms.name] || byCode[codeOf(ms.name)]);
  const parkedTickets = [];
  const unphased = [];
  const modAgg = {}; // per-module rollup (orthogonal to milestones)
  ...
  for (const i of nodes) {
    ...
    // module health — counts every non-parked ticket, regardless of milestone
    const mk = t.mod || 'unlabeled';
    const ma = modAgg[mk] || (modAgg[mk] = { name: mk, openPts: 0, donePts: 0, openCount: 0, doneCount: 0 });
    if (done) { ma.donePts += t.est; ma.doneCount++; } else { ma.openPts += t.est; ma.openCount++; }
    const g = matchMilestone(i.projectMilestone);
    if (!g) { if (!done) unphased.push(t); continue; } // OPEN in-project ticket with no matching milestone (config drift / unassigned); old completed tickets legitimately predate milestones
  ...
  const modules = Object.values(modAgg)
    .map((a) => ({ ...a, totalPts: a.openPts + a.donePts }))
    .sort((x, y) => y.openPts - x.openPts || y.totalPts - x.totalPts); // worst (most open) first
  ...
  return {
    ...
    unphased, // tickets in-project with no matching milestone — surfaced as a board warning
    total, donePts, scope,
...
// Cross-project Launch rollup: a single points scope that unions an entire project
// (cfg.macProject — Mac v1, included wholesale) with any ticket carrying cfg.launchLabel
// (the curated Cloud subset). Produces one aggregate burndown to cfg.target, plus a
// per-source breakdown and a module roll-up. Shape mirrors buildModel's points fields
// (total/donePts/scope/pct + velocity/target/startDate) so snapshot + log-history work
// unchanged; it carries `breakdown` instead of milestone columns.
async function buildLaunch(cfg) {
  const q = `query{ issues(first:250, filter:{ team:{key:{eq:"${cfg.team}"}}, state:{type:{nin:["canceled","duplicate"]}}, or:[ {project:{id:{eq:"${cfg.macProject}"}}}, {labels:{name:{eq:"${cfg.launchLabel}"}}} ] }){ nodes{ number estimate state{type} project{id} labels{nodes{name}} } } }`;
  const data = await post(q);
  const seen = new Set(), srcAgg = {}, modAgg = {};
  let total = 0, donePts = 0, openCount = 0, doneCount = 0;
  for (const i of data.issues.nodes) {
    if (seen.has(i.number)) continue; // a Mac v1 ticket could also carry the launch label
    seen.add(i.number);
    const done = i.state.type === 'completed';
    const est = i.estimate || 0;
    const src = i.project && i.project.id === cfg.macProject
      ? { name: 'Mac v1', slug: 'mac-v1' } : { name: 'Cloud (launch)', slug: 'pleach-cloud' };
    const sa = srcAgg[src.name] || (srcAgg[src.name] = { name: src.name, slug: src.slug, openPts: 0, donePts: 0, openCount: 0, doneCount: 0 });
    const mk = moduleOf(i.labels.nodes.map((x) => x.name)) || 'unlabeled';
    const ma = modAgg[mk] || (modAgg[mk] = { name: mk, openPts: 0, donePts: 0, openCount: 0, doneCount: 0 });
    if (done) { donePts += est; doneCount++; sa.donePts += est; sa.doneCount++; ma.donePts += est; ma.doneCount++; }
    else { total += est; openCount++; sa.openPts += est; sa.openCount++; ma.openPts += est; ma.openCount++; }
  }
  const scope = total + donePts;
  const breakdown = Object.values(srcAgg).map((a) => ({ ...a, totalPts: a.openPts + a.donePts })).sort((x, y) => y.openPts - x.openPts);
  const modules = Object.values(modAgg).map((a) => ({ ...a, totalPts: a.openPts + a.donePts })).sort((x, y) => y.openPts - x.openPts);
  return {
    slug: cfg.slug, title: cfg.title, mode: 'launch',
    startDate: cfg.startDate, target: cfg.target, velocity: cfg.velocity,
    total, donePts, scope, pct: scope ? Math.round((donePts / scope) * 100) : 0,
    openCount, doneCount, totalCount: openCount + doneCount,
    breakdown, modules,
  };
}
...
module.exports = { post, buildModel, buildLaunch, buildPortfolio, PROJECT_SLUG };
