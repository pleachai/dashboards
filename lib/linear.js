...
// Returns the render-ready model for a dashboard config.
async function buildModel(cfg) {
  const q = `query{ issues(first:250, filter:{ team:{key:{eq:"${cfg.team}"}}, state:{type:{in:["started","backlog","unstarted"]}} }){ nodes{ number title estimate projectMilestone{name} labels{nodes{name}} } } }`;
  const data = await post(q);
  const nodes = data.issues.nodes;
  const parked = new Set(cfg.parkedNumbers || []);

  const order = Object.keys(cfg.milestones);
  const groups = order.map((name) => ({ name, ...cfg.milestones[name], tickets: [] }));
  const byName = Object.fromEntries(groups.map((g) => [g.name, g]));
  const parkedTickets = [];

  for (const i of nodes) {
    const t = { n: i.number, title: i.title, est: i.estimate || 0, mod: moduleOf(i.labels.nodes.map((x) => x.name)) };
    if (parked.has(i.number)) { parkedTickets.push(t); continue; }
    const g = i.projectMilestone && byName[i.projectMilestone.name];
    if (g) g.tickets.push(t);
  }
  for (const g of groups) { g.tickets.sort((a, b) => a.n - b.n); g.pts = g.tickets.reduce((s, t) => s + t.est, 0); }
  ...
  return {
    ...
    total: groups.reduce((s, g) => s + g.pts, 0),
  };
}
